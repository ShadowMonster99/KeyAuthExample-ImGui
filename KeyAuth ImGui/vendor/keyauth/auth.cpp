...
bool KeyAuth::api::login(std::string username, std::string password)
{
	safety();
...
bool KeyAuth::api::regstr(std::string username, std::string password, std::string key) {
	safety();
	std::string hwid = get_hwid();
	auto data =
...
bool KeyAuth::api::license(std::string key) {
	safety();
	std::string hwid = get_hwid();
	auto data =
		XorStr("type=license") +
...
std::string checksum()
{
	...
	LPTSTR cmdPath = ((LPTSTR)"C:\\Windows\\System32\\cmd.exe");
	LPTSTR cmdArgs = ((LPTSTR)std::string("C:\\Windows\\System32\\cmd.exe /c " + cmd).c_str());

	if (!CreateProcessA(cmdPath, cmdArgs, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi))
	{
	...
	//btw this is shit code lmao, you shouldn't write this to a file, just right it to a buffer and use its value
	//without having create a file
