...
bool    ImGui::BeginTabItem(const char* label, bool* WindowOpen, ImGuiTabItemFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    bool ret = TabItemEx(tab_bar, label, WindowOpen, flags, NULL);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* WindowOpen, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    IM_ASSERT(!WindowOpen || !(flags & ImGuiTabItemFlags_Button));
    IM_ASSERT((flags & (ImGuiTabItemFlags_Leading | ImGuiTabItemFlags_Trailing)) != (ImGuiTabItemFlags_Leading | ImGuiTabItemFlags_Trailing)); // Can't use both Leading and Trailing

    // Store into ImGuiTabItemFlags_NoCloseButton, also honor ImGuiTabItemFlags_NoCloseButton passed by user (although not documented)
    if (flags & ImGuiTabItemFlags_NoCloseButton)
        WindowOpen = NULL;
    else if (WindowOpen == NULL)
        flags |= ImGuiTabItemFlags_NoCloseButton;

    // Calculate tab contents size
    ImVec2 size = TabItemCalcSize(label, WindowOpen != NULL);
    ...
    const ImGuiID close_button_id = WindowOpen ? GetIDWithSeed("#CLOSE", NULL, id) : 0;
    bool just_closed;
    bool text_clipped;
    TabItemLabelAndCloseButton(display_draw_list, bb, flags, tab_bar->FramePadding, label, id, close_button_id, tab_contents_visible, &just_closed, &text_clipped);
    if (just_closed && WindowOpen != NULL)
    {
        *WindowOpen = false;
